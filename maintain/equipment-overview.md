...
The equipment library provides the foundation for aircraft and equipment management in GlidingApp. **Equipment** represent all physical assets used in club operations, such as aircraft, winches, towcars, hangars, and other equipment. Each piece of equipment serves as the central record linking flight logs, technical documentation, maintenance schedules, defect tracking, and work history. The system distinguishes between unlisted aircraft (minimal registration for flight logging) and full equipment records (complete maintenance tracking with parts, programs, workorders, and documents).

The equipment library follows a hierarchical data model: **Equipment** → **Parts** → **Maintenance Programs** → **Workorders** → **Documents**. Each piece of equipment serves as the root entity containing all related technical records. **Parts** represent individual components tracked within a material (aircraft fuselage, engine, propeller, winch hooks). **Maintenance Programs** define scheduled inspection and overhaul intervals for parts. **Workorders** capture maintenance work performed, linking scheduled programs or defect repairs to completed tasks. **Documents** store certificates, manuals, and technical publications with expiration tracking.

This chapter describes the overview of the Equipment module, the remaining sub-modules are described in the next chapters.

## Core Concepts

### 1. Equipment Categories

Equipment is classified into three **categories** that determine behavior, visibility, and available features:
...
Aircraft equipment receive special treatment with configuration fields, motor hour tracking, and flight integration. Rolling and other equipment use simplified tracking focused on maintenance intervals and document management without flight-related features.
...
**Listed aircraft** (full equipment records) activate the complete maintenance module. All flight history from the unlisted period transfers seamlessly during conversion. The system then enables parts library, maintenance program scheduling, workorder processing, defect tracking, document management, and team assignment. Private aircraft typically start unlisted to minimize administrative overhead for owners managing their own maintenance externally. Club aircraft and tow planes should always be entered as full equipment records to enable proper maintenance tracking and regulatory compliance.

### 3. Equipment Teams and Roles

Each piece of equipment has a dedicated **team** with members assigned specific **roles** controlling access and responsibilities. Team membership operates independently from global user permissions—a club member without global maintenance permissions can be assigned as an equipment technician for specific equipment.

| Parameter | Description | Example |
|-----------|-------------|---------|
| **Admin/Owner** | Full control over all equipment aspects including settings, data, team membership, and configuration. Only role that can modify team composition. | Equipment owner, chief of maintenance |
...
Team visibility follows equipment category rules. Club aircraft teams extend visibility to all members for flight operations. Private aircraft teams restrict visibility to assigned members only unless the "visible to all technicians" flag is enabled.

### 4. Aircraft Configuration Fields

Aircraft equipment include specialized fields integrating with flight operations and maintenance scheduling:

| Parameter | Description | Example |
|-----------|-------------|---------|
| **Equipment Visibility** | Club aircraft (category "club" or "tow") are visible to all members for flight operations. Private aircraft are only visible for team. The "visible to all technicians" flag overrides category rules to show equipment to all users with technician permissions. | Club, Private |
...
Equipment can be **archived** to create complete backup exports of all associated data. Archives include flights, parts, maintenance programs with all tasks, workorders with task completion records and uploaded files, defects with messages and attachments, documents with all versions, and hour registration records. The system generates a ZIP file with all uploaded files organized by type.

Archiving serves multiple purposes: annual record retention for regulatory compliance, equipment decommissioning documentation, or ownership transfer packages. You can **schedule ** a backup to send the sends archive files that night to a configured email. 
...
### Convert Unlisted Aircraft to Equipment with Maintenance Module

**What:** Upgrade a basic aircraft registration to a full equipment record with maintenance tracking capabilities  
...
3. Click **Convert to Equipment** button

After conversion, the equipment gains access to Parts, Maintenance Programs, Workorders, Defects, and Documents tabs. All historical flights remain linked and continue contributing to hour/launch totals.

### Add New Equipment (Aircraft, Winch, or Other)

**What:** Create a new equipment record with full maintenance module from the start  
**When:** Adding club aircraft, purchasing new equipment, or setting up maintenance tracking for rolling/other equipment  
**Permission:** "Can update equipment"
...
The system creates equipment with full maintenance module enabled. Proceed to the **Data** tab to configure team members, daily inspection requirements, and initial hour values for existing equipment. For new equipment, initial values remain at zero.
